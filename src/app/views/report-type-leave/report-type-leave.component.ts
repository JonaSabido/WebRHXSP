...
@Component({
  selector: 'app-report-type-leave',
  standalone: true,
  imports: [BreadcrumbComponent, DropdownModule, FormsModule, ButtonModule, InputTextModule],
  templateUrl: './report-type-leave.component.html',
  styleUrl: './report-type-leave.component.scss'
})
export class ReportTypeLeaveComponent extends View implements OnInit {
  module = 'Motivos de Baja'
  icon = 'pi-angle-double-down'
  prevLinks = ['Home', 'Reportes']
  activeLink = 'Motivos de Baja'
  months = months
  select_month: number
  select_year: number
  message: string
  ...
  getData() {
    ...
    this.analyticService.getTypeLeavesByMonth(this.select_month, this.select_year).subscribe({
      next: (response) => {
