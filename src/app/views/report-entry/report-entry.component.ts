...
export class ReportEntryComponent extends View implements OnInit {
  module = 'Movimientos de Alta'
  icon = 'pi-angle-double-up'
  prevLinks = ['Home', 'Reportes']
  activeLink = 'Movimientos de Alta'
  months = months
  select_month: number
  select_year: number
  message: string
  ...
  getData() {
    ...
    this.analyticService.getEntriesByMonth(this.select_month, this.select_year).subscribe({
      next: (response) => {
