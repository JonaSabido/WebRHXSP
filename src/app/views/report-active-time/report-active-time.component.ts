...
@Component({
  selector: 'app-report-active-time',
  standalone: true,
  imports: [BreadcrumbComponent, DropdownModule, FormsModule, ButtonModule, InputTextModule],
  templateUrl: './report-active-time.component.html',
  styleUrl: './report-active-time.component.scss'
})
export class ReportActiveTimeComponent extends View implements OnInit {
  module = 'Tiempos Activos'
  icon = 'pi-history'
  prevLinks = ['Home', 'Reportes']
  activeLink = 'Tiempos Activos'
  months = months
  select_month: number
  select_year: number
  message: string
  ...
  getData() {
    ...
    this.analyticService.getActiveTimesByMonth(this.select_month, this.select_year).subscribe({
      next: (response) => {
