...
export class ReportValidationComponent extends View implements OnInit {
  module = 'Validaciones'
  icon = 'pi-check'
  prevLinks = ['Home', 'Reportes']
  activeLink = 'Validaciones'
  months = months
  select_month: number
  select_year: number
  message: string
  ...
  getData() {
    ...
    this.analyticService.getValidationsByMonth(this.select_month, this.select_year).subscribe({
      next: (response) => {
